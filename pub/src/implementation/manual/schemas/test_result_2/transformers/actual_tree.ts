...
const op_cast_to_non_empty_dictionary = <T>($: _pi.Dictionary<T>): _pi.Optional_Value<_pi.Dictionary<T>> => $.is_empty() ? _p.optional.not_set() : _p.optional.set($)
...
export const Test_Group_Result: _pi.Transformer<d_in.Test_Collection_Result, d_out.Directory> = ($) => $.filter(($) => Test_Node_Result($))